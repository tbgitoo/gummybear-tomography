"""Notebook visualization helpers for particle geometry and transport pairs.

Purely diagnostic; does not affect forward-model numerics.
"""
...
from .geometry import ParticleIntersectionEvent, ParticleSet, ParticleSphere
...
def plot_sphere(
    ...
    *,
    ...
    """
    Plot an analytic particle sphere as a 3D wireframe or translucent surface.
    ...
    filled
        If True, draw a translucent ``plot_surface`` instead of a wireframe.
...
def set_axes_equal(ax: Axes3D) -> None:
    """
    Set equal scaling on all three axes so spheres appear spherical
    rather than stretched by matplotlib's default autoscaling.
    """
...
def plot_segment(
    ...
    """Draw one 3D line segment between two points."""
